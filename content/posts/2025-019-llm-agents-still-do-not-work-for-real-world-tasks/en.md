...
The result — a fail — the final configs in both cases managed to be, **simultaneously**:
...
**Second task** — prepare a frontend app template for [Vue 3](https://vuejs.org/): create an app, install [Tailwind CSS](https://tailwindcss.com/), add one specific library — [Dockview](https://dockview.dev/) — build a couple of dummy components. This time I used Copilot Agents. I just wanted to save time, plus this is a lowest-priority project I dream of doing in a vibe-coding style, when it becomes possible :-)

Result — another fail — the packages were installed, but I wouldn't call them properly configured; the required GUI simply didn't render. Copilot did, however, produce a pretty README with a screenshot of an empty page and, amusingly, tried to convince me that although the screenshot is blank, everything actually works — the correct screenshots just got lost somewhere in the tmp directories.

Both are straight-up routine tasks — work for a patient mid-level dev, maybe even for junior — you don't need to be a genius to get them done.
