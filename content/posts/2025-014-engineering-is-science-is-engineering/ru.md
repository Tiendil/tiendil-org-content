...
Де юре, с философской точки зрения, инженерия и наука производят разное знание:

- Наука производит (по крайней мере старается) концептуально новое знание о мире — как мир работает.
...
- Здание, которое будет устойчиво к землетрясениям в 7 баллов, будет иметь уровень энергопотребления не выше X, и будет пригодно для проживания при температурах от -30 до +50 градусов. При условии, что оно будет построено на такойто почве, из таких-то материалов, с такими-то технологиями.
...
Наука сильнее ориентирована на объяснительную составляющую модели (почему что-то работает так, а не иначе), инженерия — на предсказательную (как будет вести себя конкретная штука в конкретой среде).
...
В случае науки неизвестность определяется и неполной информацией о мире (например, в физике) так и изменичивостью мира (например, в биологии или социальных науках).
...
Если ваш проект длится пару лет, то при начале следующего вы вынуждены полностью пересобирать весь стек технологий, чуть ли не от железа. Это не значит, что вы полностью меняете стек, но существенную его часть приходится заменять/обновлять (если вы хотите оставаться на фронтире и делать топовые проекта), а значит, требуется заново получать знания о взаимодействии компонентов всего стека — ставить эксперименты, ошибаться, учиться.
...
Стороннему наблюдателю (да и многим изнутри профессии) может казаться, что экспермент занимает довольно малую часть инженерной работы. Но это не так. Просто на инженеров сильнее давят экономические и временные факторы, из-за чего форма эксперимента слегка меняется.
  ...
  - Принятие решений научное: выдвигаем гипотезу, стави эксперимент (если есть возможность), наше нарешение может быть ошибочным и это нормально. Нужно было документное хранилище, взяли монгу, окзалось что постгря лучше подходит. Выбрали ECS для скорости, оказалось что сущности гиперсвязаны и доступ к ним рандомый, что сводит преимущеста ECS на нет. Часто инженеры действуют без эксперимента, но не потому что он прям не нужен, а потому что он экономически не эффективен — дешевле исправить последствия ошибки, чем делать эксперимент. Когда последствия ошибки дорогие, всегда ставится эксперимент.
...
В моей ранней карьере было несколько моментов, когда я ловил разного рода инсайты, которые существенно перестраивали мой взгляд на профессии. Иногда это случалось после собственных размышлений, иногда после разговора с кем-то из коллег.

Один из таких моментов как раз связан с тем, что мой лид в Wargaming несколько раз отправлял меня ставить эксперимент и собирать данные вместо разведения болтологии и споров о том, как правильно что-то сделать. Спасибо, Бао! Было полезно.

Почему это до меня не донесли, например, в университете, я не представляю.
///

<!-- TODO: добавить ещё пунктов -->
...
- Наука сильнее ориентирована на объяснительную составляющую модели (почему что-то работает так, а не иначе), инженерия — на предсказательную (что будет работать в заданной среде). Сейчас это размывается, так как в науке (например, в физике) зреет кризис объяснительной силы моделей и происходит (на мой взгляд) медленный сдвиг в сторону чисто предсказательных моделей.
...
- Для инженеров (и любой другой творческой работы) нет инструкций, есть рекомендации. В момент когда рекомендации превращаются в инструкции, эффективность инжера падает радикально.
