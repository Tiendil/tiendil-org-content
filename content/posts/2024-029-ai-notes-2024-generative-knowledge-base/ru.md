...
ТУДУ: «веростностное продолжение» — вот это бы убрать?
ТУДУ: вероятностное продолжение vs вероятностный ответ?

На мой взгляд, подрывная технология — это новый тип баз данных, основаный на статистике и хранящий семантику вместо реальных данных.
...
- **Возможность корректировки формы ответа**. Самый простой пример — чаты. Дообучая `GKB` на примерах диалогов, мы получаем базу данных, которая отвечает продолжением диалога.

## Важно разделять свойства `GKB` и свойства её конкретных реализаций

Для примера давайте посмотрим на технлогию классических реляционных баз данных. Они написаны  на разных языках, дают разные гарантии атомарности, сохранности данных, конкуретности, по-разному соотносятся с [CAP-теоремой](https://ru.wikipedia.org/wiki/Теорема_CAP), имеют разные профили производительности и так далее. Но концептуально обладают общими базовыми свойствами: есть таблицы, индексы, SQL, реляционная алгебра, транзакции, etc. На этих свойствах, а не на особенностях реализации, сейчас и стоит большая часть нашего IT мира.

Также обращу внимание на то, что атомарные технологии под `GKB` сейчас вполне конкретные (большие данные, нейронки, специализированные видеокарты), но концептуально она могла бы быть построена и на других технологиях. Например, на голосовании 100500 человек, или на [генетическом программировании](https://ru.wikipedia.org/wiki/Генетическое_программирование) поверх квантовых компьютеров. Просто так эволюционно сложилось, что в нашем конкретном случае эту базу собрали на нейронках. Эти альтернативные реализации имели бы другие наборы второстепенных преимуществ и недостатков, но для их успеха потребовался бы тот же набор базовых свойств.

Поэтому я постарался разделить базовые свойства `GKB`, которые описал выше, и свойства производные от технологий на уровень ниже.

Вот некоторые из производных.
...
**Сжатие информации в векторную форму** (эмбединги). На мой взгляд это типичная особенность реализации. Беспорно удобная и полезна, но нет гарантии, что эмбединги останутся в явном виде — «легко» может появиться альтернативное железо на каких-нибудть полях или квантовой запутанности в котором нельзя будет снять данные с промежуточных слоёв без разрушения целостности базы.

**Добавление данных через обучение с [градиентным спуском](https://ru.wikipedia.org/wiki/Градиентный_спуск)** — также артефакт текущей реализации. Альтернативой мог бы стать, например, какой-нибудь эволюционный отбор. Правда тут я не возьмус утверждать, что у нас есть реальная альтернатива. Возможно это действительно самый математически оптимальый способ оптимизации моделей.
...
- Мы можем запросить что-то из неё и получить вероятностное продолжение запроса.
- Продолжение может быть неправильным, поэтому мы его проверяем, дублируем, ранжируем ответы и занимаемся всеми другими штуками, которые позволяют обходить проблемы статистики.

На текущий момент я не слышал о каком-либо другом подходе к использованию нейронок.
...
- Если кто-то пытается использовать нейронки не как `GKB`, например создать сильный ИИ или «автономного работника» только на текущих нейронках, — скорее всего люди не понимают что делают и у них ничего не получится.

На всякий случай отмечу, что всякие copilot и прочие ассистенты подпадают как раз под третий пункт, а не под четвёртый. Так как типичная функция ассистента — извелечение информации, которую вы потом анализируете.

Поэтому же некоторые люди, которые этого не понимают, не могут извлечь пользы от асистентов, так как пытаются взаимодействовать с ними не как с вероятностной базой знаний, а как с людьми.

## `GKB` и сильный искуственный интеллект

Если кратко, то человек — далеко не только база знаний, но и набор логики поверх неё.

Соответственно, расчитывать на то, что какая-нибудь GPT-10 обретёт сознание пока не стоит.

`Generative Knowledge Base` может быть важным куском/модулем для создания сильного ИИ, но всего лишь одним из множества.

Часть из этого множества у нас может быть уже под рукой: разного рода математика и логика, исчисление предикатов, логическое/символьное/функциональное/императивное программирование, традиционные базы данных, семантические сети, etc.
