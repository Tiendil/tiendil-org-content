...
В этом плане, модель o1 выглядит как попытка «задёшево» двинуть универсальные LLM одновременно по путям 3 и 4. Получилось лучше, чем GPT-4, но не на порядок. Например, я всё ещё пользуюсь своими [кастомными GPT-шками]{post:my-gpts} вместо o1 для некоторых задач.
