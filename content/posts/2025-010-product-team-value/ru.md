...
<!-- TODO: надо поправить утверждение и далее всю логику -->
<!-- Инверитровать главы во второй половине текста? -->
<!-- Причинное окружение конечных метрик, как рычаги -->
<!-- Конечные метрики влияют на конечные и это нормально, т.к. их связь находится во внешней среде по отношению к нашему продукту. Для примера, мы можем повлиять на то как быстро саппорт отвечает пользователю и процент отказа от подписок — это свойство нашего продукта, но сам пользователь вне нашего контроля, а значит и принятие им решения об отказе от подписки тоже. -->
<!-- То есть у нас два типа конечных метрик: одни управляются полностью нами, другие через окружающую среду. Но это не всегда, т.к. есть и чисто внутренние метрики, которые мы хотим видеть как конечные. Следовательно, нам правильнее говорить о **ближайшем причинном окружении**. -->

Опираясь на эвристики, мы можем утверждать, что **нас интересуют гипотезы об измеримом изменении единичных конечных свойств продукта и их измеримом влиянии на другие (по-возможности единичные) конечные свойства продукта.** Например, всё тот же случай с ускорением работы поддержки для уменьшения процента отказов от подписки.
...
**Целевые показатели** — это вопрос видения продукта, стратегии его развития и действий иммунной системы, они отображают наши чцели, ценности и риски.
...
<!-- TODO: мы измеримо работает с внешними гипотезами, но внутрянку меняет не обязательно измеримо? Внутрянка самоорганизуется.-->
