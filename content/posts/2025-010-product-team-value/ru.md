...
<!-- TODO: надо ли упоминать про влияние рычагов на целевые метрики через окружающую среду? -->

Можно утверждать, что **нам интересны метрики поведения продукта во внешней среде** — количественное измерение того он взаимодействует с ним. Обычно это метрики взаимодействия с пользователями, как пользователи взаимодействуют с ним.
...
<!-- Рекурсивность гипотез:

Мы не пытаемся управлять выручкой через найм или увольнения людей. Те же увольнения могут быть частью наших действий, как следствия имплементации гипотезы, но не частью самой гипотезы.

Плохая гипотеза: Мы увеличим выручку на 10% за 3 месяца, если наймём 5 новых разработчиков.

Хорошие гипотезы:

1. «Мы увеличим выручку на 10% за 3 месяца, если реализуем фичу X» => мы запускаем «проект X» <- это наш целевой показатель для подсистемы.
2. «Чтобы реализовать «проект X» нам необходима команда из 5 человек» => мы запускаем «активность по поиску 5 человек» <- это наш ещё более частный целевой показтель.
3. «Нанять 5 человек в текущей ситуации выгоднее, чем снять 5 человек с других задач» => мы запускаем «активность по найму 5 человек» <- это наш ещё более частный целевой показтель.
4 …
