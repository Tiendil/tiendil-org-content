...
Опираясь на эвристики, мы можем утверждать, что **нас интересуют гипотезы об изменении единичных конечных свойств продукта через измеримые единичные изменения свойств из их ближайшего причинного окружения**. Например, всё тот же случай с уменьшением процента отказов от подписки через ускорение работы поддержки.
...
Суть не в том, чтобы найти именно изменение `X->Y`, а в том, чтобы среди всех возможных следсвий единичного изменения `X` выделялось одно или несколько существенных и множество мылых и/или незначимых.
