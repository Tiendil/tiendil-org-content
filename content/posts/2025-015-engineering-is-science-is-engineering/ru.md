...
Инженерное описание артефакта, по сути, является частной моделью окружающей реальности либо явления в ней. Как и любая модель она имеет область применения, ограничения, допущения, ограничения по точности и прочее.
...
Даже спецификация гайки, формально, описывает её в 4D, так как неявно учитывает изменение свойств гайки со временем (например, износ) в виде указания на материалы и условия эксплуатации.
...
Однако для нужд данного поста подобный свойства моделей не существенны — важно само наличие моделей.
///

Концептуально тем же самым занимается и наука — создаёт модели окружающей реальности, только более глобальные.
...
Пример научной модели — [теория всего](https://ru.wikipedia.org/wiki/Теория_всего).

Пример инженерной модели — чертёж конкретного моста.
