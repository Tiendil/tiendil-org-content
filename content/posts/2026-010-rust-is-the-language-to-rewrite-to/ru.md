...
Я использовал Rust не больше года; для прототипирования и экспериментирования; в геймдеве.
...
В тоже время у меня достаточно опыта, чтобы экстраполировать мой личный опыт на более общие случаи и сформировать своё субъективное мнение о Rust. Этим я в этом эссе и займусь.
...
К слову, по моим наблюдениям Rust стал последним языком, который научились осиливать LLM.
...
Поскольку Rust мешает писать ~~простой~~ плохо код и, одновременно, даёт инструменты для написания ~~сложного~~ хорошего кода, получается, что он сдвигает баланс в сторону более сложного кода.

А как известно, нет предела совершенству. Играть трейтами, организовать абстракции удобно и увлекательно — это как решать олимпиадые задачки.
...
У меня, кстати, примерно такие же впечатления.
...
### Надёжность Rust может быть иллюзией
...
То есть надёжность и безопасность софта, который пишут на Rust может быть следствием того, что

1. Мы в целом начали писать софт более качественно, выбирать более правильную архитектуру.
2. В случае переписывания на Rust, все существенные ошибки уже сделаны разработчиками оригинального софта, поэтому переписывающим разработчикам достаётся изначально менее сложная и рисковая задача.

### Rust выдвигает высокие требования к способностям программиста

Команда разработчиков на Rust будет крутой и дорогой — этот вариант подходит не всем компаниям и не всем проектам. Ошибки в архитектуре софта на раст будут происходить реже, но могут стоить существенно дороже из-за своей возможной «фундаментальности» на уровне проекта.

ИИ, в теории, может снизить требования к уровню разработчиков, но пока что это скорее теория. [Мои личные впечатления от coding agent]{post:notes-on-coding-agents} говорят, что те отлично справшляют с кодом, но очень плохо с архитектурой и долгосрочным планированием. Большинство проблем в вашем проекте на Rust будет как раз из-за ошибок в архитектуре.
...
- Как бывшему олимпиаднику, мне Rust нравится, потому что он даёт решать прикольные задачки. Примерно уровня задач на шаблоны в C++ — по ощущениям одно и то же.
- Идеалогически мне Rust не нравится, так как он ограничивает свободу программиста.
- В кроткосрочной и среднесрочной перспективе Rust, скорее всего, будет отъедать весь рынок «стандартного» и «foundational» софта, чтобы «стандартный» и «foundational» в этом случае не значили.
- В долгосрочной перспективе я вижу риск для Rust закопать себя точно также, как себя закапывает C++ последние 20 лет.
...
Если вы хотите копнуть немного глубже, то вот несколько полезных ссылок:
