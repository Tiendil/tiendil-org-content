...
Я выбирал маяки по жанру и значимому опыту, который получают игроки. Тут можно пофилосовстовать и найти кучу альтернатив. Например, раскопать какую-нибудь статистику Steam или подключить ИИ, кластеризацию и прочий дата-анализ.
...
Но точные данные нам и не нужны (типо все предыдущие наши расчёты были точны, ага), а примерная информация неожиданно легко нашлась в Википедии и новостных ресурсах.
...
Мы разобьём DLC на два типа: платные и бесплатные и будем их чередовать.
...
- Для Early Access снимаем геймплей видео и «немного» тратимся на превлечение хардкорных игроков в стратегии, чтобы культивировать из них ядро сообщества.
- На Release снимаем крутое видео с идеей игры и вливаем деньги в широкую аудиторию, чтобы набрать игроков.
- На выпуске бесплатного DLC возвраащаем старых игроков и немного привлекаем новых, целясь на особенности конкретного DLC.
...
- Краткая выдержка спервого листа — срез самых важных параметров в моменты ключевых событий разработки — для инвесторов и для красивой презентации.
...
`[4] Standard Edition Price $` — Цена базовой версии игры. Её выбираем балансируя между нашей жадностью и ценой продажи наших игр-маяков на их релизе.
...
/// warning | Осторожно!
Если вы ничего не делали до релиза, то тут у вас будет 0.
///

`[7] Game Purchases Boost `— Количество покупок игры, которые мы ожидаем от наших маркетинговых усилий. Этот параметр мы не расчитываем, а задаём.

**Причины выбора такого подхода и его альтернативы будут разобраны чуть позже**, когда дойдём до расчётов трат на маркетинг.
...
`[8] Game Purchases Boost Tail` — Игроки, которые привлекаются как хвост/эхо ваших маркетинговых активностей. Рассчитываю как `1/3` от эффективности маркетинга в предыдущий месяц. `1/3` взял из графиков SteamSpy (как быстро уменьшается скорость прироста аудитории).
...
К расчёту скидок может два подхода.
...
«Терять» тут в кавычках, очевидно, что это скорее «находить» (так как без скидок игру и не купили бы), но будем использовать термин «терять», так как лучше соответствует логике расчёта.
...
`[41] Steam Fees` — Комиссия Steam. Стандартные `30%`, но говорят можно договориться :-D

`[42] Revenue After Steam Fees` — Наш доход после комиссии Steam.

`[43] Total Revenue After Steam Fees` — Наш суммарный доход после всех комиссий, включая текущий месяц.

### Траты на разработку

Так часть модели, для которой мы так долго считали roadmap и состав команды.

`[44] Team Size` — Размер команды намесяц расчёта.
...
Часто roadmap и даже концепцию продукта проще загнуть под команду, чем собрать команду под продукт. Например, когда я работал над [Сказкой](https://the-tale.org), я поменял стилистику игры с юморной на серьёзную, потому что нашёл геймдизов который круто умели в серьёзный нарратив. В случае крупных проектов это, конечно, не так актуально, но всё равно следует учитывать.

Утрируя, вы не знаете кто вам подвернётся при найме. Крутой мотивированый спец может повернуть всю вашу разработку в совершенно новое и более крутое русло только за счёт своего опыта в уникального видения. Отбрасывать эту возможность не стоит.
///

`[46] Sallary Indexation` — Индексация зарплат. Мне кажется это почти никто не закладывает, а зря. На мой взгляд это обязательный элемент соглашения в наши дни. Так сказать, элемент рабочей этики.
...
**Это минус нашей финансовой модели.**
...
1. Оперируя вишлистами мы усложняем нашу модель. Появляется два неопределённых этапа: набор вишлистов и конверсия их в покупки. Поведение нашего продукта, Steam и игроков на кадом из этих этапов слабо предсказуемо, ака, случайное. На мой взгляд, модель с двумя «случайными» параметрами будет давать результаты хуже модели с одним «случайным» параметром, которую используем мы.

2. Steam и геймдев в целом — очень динамические среды. Любое обновление Steam легко пошатает все расчёты на вишлистах. Вот, например, недавно улучшили функциональность демо версий, как следствие, поменялась логика продвижения игр, а раздел `New & Trending` полностью изменил своё содеражание — теперь он забит демками. Соответственно, если у вас были данные, например, о стоимости вишлиста в прошлом году, то сейчас они уже не актуальны.

3. Мне не удалось найти достоверных утверждений про «стоимость одного вишлиста», а оценки коэфициентов конверсии из вишлистов в покупку от источника к источнику могут отличаться в 1.5-2 раза. Тем боле, оценки будут отличаться для разных жанров, стилей и размеров игр. Статистики для стратегий не то, чтобы много, скорее нет, потому что самих стратегий мало.
...
В нашей модели мы выберем самый простой (и от того надёжный) подход — будем напрямую оценивать стоимость приклечения покупателя нашей игры, ака [CPI](https://ru.wikipedia.org/wiki/Cost_per_install).
...
- Оценка `CPI` будет точнее конверсий вишлистов, так как напрямую и жёстко ограничивается рынком: бюджетами, готовностью людей вестись на реклмау, etc. Компании, сделавшие плохой маркетинг, уходят с рынка. Если мы будем использовать данные успешных продуктов, мы будем получать корректные данные

`CPI` для базовой игры мы будем оценивать просто:

- Бюджет на маркетинг игр принимаем равным половине их доходов (revenue).
- Делим бюлжет маркетинга на оценку количества покупок, получаем `CPI`.

Наш `CPI` (для базовой игры) установим примерно посередине между играми-маяками.

### Траты на маркетинг

Итак, возвращаемя к самой большой области наих расходов. Согласно финальным расчётам нашей модели к релизу на маркетинг уйдёт раза в 2.5 больше, чем на разработку :-D

`[54] Marketing Wishlists Cost` — Оценка минимальной перманентной ежемесячной активности по «лёгкому» пиару игры.
...
Проблема с ним в том, что в отличии от `CPI` для основной игры, оценить `CPI` для DLC непонятно как. И его колебания очень сильно влияют на модель.
...
2. Мы хотим приввлекать `Y` покупателей, поэтому учётом `CPI` мы будем просить у инвесторов `X$` в месяц на маркетинг».
...
`[64] Month Gross Revenue` — Наш доход в месяц, после всех пиходов и расходов.

`[65] EBITDA`— [EBITDA](https://ru.wikipedia.org/wiki/EBITDA) как она есть — наши деньги до вычета непрямы расходов (налоги, амортизация, проценты, etc).

## Что делать, когда закончили финансовую модель

После того как вы вобьёте последнюю формулу, растянете колонки на нужное количество месяцев, разукрасити всё, вы посмотрите на итоговые числа и увидите, что там фигня какая-то неприбыльная. Или, наоборот, сильно подозрительно прибыльная.
...
- **Краткая стратегия компании** — стратегию вы продумываете не в какой-то конкретный момент, а всё время пока работаете над концептами, презентациями, и прочи. Потом вы просто записываете информацию, которая уже давно сидит в голове.
- **Табличка игр-маяков** — ±2 дня. Долго пытался понять что и где искать, когда разобоался, пошло быстро.
