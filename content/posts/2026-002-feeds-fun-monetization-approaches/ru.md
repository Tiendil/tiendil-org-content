...
**Во-первых**, так как мы рассматриваем худший сценарий и маржа заложена и в подписку и в докупку токенов, то разумно предполагать, что пользователи каждого тира подписки будут максимально использовать свои квоты, но не будут докупать токены сверх них.

Использование квот до их исчерпания будет уменьшать доходы, а вот докупка токенов после исчерпания квот — будет их увеличивать. Поэтому наш минимум дохода с пользователя находится в точке максимального использования квот.

**Во-вторых**, некоторые группы пользователей могут попадать целиком в однин тир подписки:

- если максимальное потребление группы пользователей меньше квоты базовой подписки, то вся группа идёт на базовую подписку;
- если минимальное потребление группы пользователей больше квоты профессиональной подписки, то вся группа идёт на профессиональную подписку.
