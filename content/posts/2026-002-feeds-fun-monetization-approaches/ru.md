...
В целом, по здравому рассуждению, выбор подхода к монетизации на старте проекта — это больше про пользовательский опыт, чем про конретные суммы. Конкретные числа (включая цены для пользователей) куда проще оптимизировать итерационно, когда есть реальная статистика поведения пользоваетелей. А вот изменить базове предложение ценности и пользовательский опыт — гораздо сложнее. Разница примерно как между «поменять пару числе в конфигах» vs «переписать здоровый кусок бизнес-логики и UI».
