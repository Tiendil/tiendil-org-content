...
Понятно, что пользовательская база будет как-то распределяться по этим группам. Поскольку данных откровенно недостаточно, мы разобьём пользователей [по принципу Парето 80/20](https://ru.wikipedia.org/wiki/Закон_Парето), так как он встречается абсолютно везде.
