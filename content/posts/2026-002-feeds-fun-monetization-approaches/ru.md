...
4. Наша аудитория это 80% начинающих пользователей (25 новостей в день), 16% продвинутых (100 новостей в день) и 4% хардкорных (1000 новостей в день).
...
**Во-первых**, так как мы рассматриваем худший сценарий и маржа заложена и в подписку и в докупку токенов, то разумно предполагать, что пользователи каждого тира подписки будут максимально использовать свои квоты.

Использование квот до их исчерпания уменьшает доходы, а вот докупка токенов после исчерпания квот их увеличивает. Поэтому наш минимум дохода с пользователя находится в точке максимального использования квот.

То есть:

- Если пользователь находится на подписке которая полностью покрывает его потребление, то он использует всю квоту подписки.
- Если пользователь находится на профессиональной подписке и его потребление больше квоты, то он всё равно использует только квоту профессиональной подписки.

**Во-вторых**, мы будем счатать, что пользователь всегда выбирает самую оптимальную подписку для себя с точки зрения цены.

**В-третьих**, чтобы не заморачиваться, мы просто зададим конкретное потребление для каждого пользователя.
...
3. Для каждого пользователя в группе зададим конкретное потребление новостей в день с помощью лог-равномерного распределения между минимальным и максимальным потреблением.
