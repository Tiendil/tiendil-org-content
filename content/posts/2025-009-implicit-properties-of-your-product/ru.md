...
Представим, вы разрабатываете [SaaS](https://en.wikipedia.org/wiki/Software_as_a_service) — тамагочи todo-лист для олигархов, или даже что-нибудь менее полезное.
...
Кроме очевидных свойств вроде конкретной функциональности, внешнего вида и золотого айфона в комплекте, ваш продукт обладает множеством неочевидных свойств, которые также влияют на его успешность.

Пользователи многое ожидают от продукта неявно, вот несколько примеров:
...
Ваши пользователи могут даже (подсознательно) расчитывать на конкретные эмоции, которые продукт должен вызывать. Это касается не только игр, в которых эмоции — один из ключевых компонентов (сравните, для примера, эмоции от [Dark Souls](https://en.wikipedia.org/wiki/Dark_Souls), [Doom](https://en.wikipedia.org/wiki/Doom_(1993_video_game)) и [The Sims](https://en.wikipedia.org/wiki/The_Sims))), но и бизнес-приложений, которые могут давать ощущения контроля (, надёжности, осведомлённости, etc.

Эти свойства имеют разную степень влияния на успешность продукта. К тому же, сила влияния зависит от типа продукта, от рынка, от среза пользователей и от множества других факторов.

Однако, пользователи всё же ожидают их наличие — в той или иной степени.

Поэтому, де-факто, неявные свойства продукта являются такой же полноправной его частью, как и та же функциональность. А значит, они входят в зону ответственности разработчиков продукта.
...
Есть интересные случаи, когда некоторые свойства удаётся полностью игнорировать, например:
...
- Те же монопольные продукты, пожалуй, один из немногих случаев, когда эмоции пользователя можно полностью игнорировать. Если налоги можно заплатить только через ваш софт, то пользователи за него заплатят, даже если у них при его использовании будут возникать позывы к суициду.

Но это не наш путь. Мы должны стремиться делать мир лучше, а не как вот это вот всё.
///

Соответственно, мы должны оценивать и учитывать их влияние на успешность продукта, мы должны выделять ресусы на их реализацию, иными словами, мы должны управлять ими.

Чтобы понять чем конкретно и как надо управлять, давайте посмотрим на продукт как на эволюционирующую 4D систему — почему бы и нет :-D

TODO: надо ли начинать от 4D системы, или заканчивать на ней?
...
!!! И вот что у меня вызывает дикое непонимание и неприятие, так это постоянные аналогии разработки софта с физическим конвеерным производством. Во всех, блин книгах. Когда это концептуально неверно. И да, в последние годы даже физическое производство двигается в эту же сторону, но софт же всегда был там, при этом им всегда пытались управлять как физическим конвейером, а не как живой эволюционирующей системой.
