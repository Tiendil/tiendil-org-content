...
Под публичностью в данном тексте я имею в виду глобальную публичность, то есть доступность артефакта для всех, кто может быть заинтересован в нём. Это не обязательно предполагает открытую лицензию (например, open-source), хотя открытая лицензия часто — естественный выбор, который упрощает многие вещи.
...
В IT с этим сложнее, так как [конвейерная культура]{post:no-instructions-for-engineering} не позволяет людям открывать результаты своего труда. Однако это не значит, что этот подход не работает — множество разработчиков создают публичные артефакты, некоторым даже везёт делать это в рабочее время.
